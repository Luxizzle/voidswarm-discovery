...
module.exports = class VoidswarmDiscovery extends EventEmitter {
  constructor(options = {}) {
    ...
    if (this.options.noAnnounce === false) {
      this.announceInterval = setInterval(this.announce.bind(this), this.options.interval)
    }
  }
  ...
  onMessage(message, remote) {
    ...
    if (message.toString() === this.identifier) {
      if (this.nodes.has(address)) {
        ...
        this.nodesTimeout.set(address, setTimeout(this.timeout.bind(this), this.options.interval*2, address))
      } else {
  ...
  announce() {
    ...
    this.socket.send(message, 0, message.length, this.options.port, this.options.address)
  }
